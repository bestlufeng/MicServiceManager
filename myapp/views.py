...
def kubenetes_deployment(request):
    ...
    if request.method == "GET":
        ...
        kube_data = requests.request(method=request.method,url=url,headers=kube_headers)
        response = {
            "result" : True,
            "data": deployment,
    ...
    if request.method == "POST":
        ...
        response = requests.request(method=request.method,url=url,headers=kube_headers,data=json.dumps(deployment))
...
def kubenetes_service(request):
    ...
    if request.method == "GET":
        ...
        kube_data = requests.request(method=request.method,url=url,headers=kube_headers).text
    ...
    if request.method == "PUT":
        url = str(kube_server+"/api/v1/namespaces/"+ namespace+ "-" + appEnv  +"/services/" + appName)
        response = requests.request(method=request.method,url=url,headers=kube_headers,data=json.dumps(service))
        return HttpResponse(response)
    if request.method == "POST":
...
def config_map(request):
    ...
    if request.method == 'POST':
        ...
        response = requests.request(method="POST",url=url,headers=kube_headers,data=json.dumps(data))
        return HttpResponse(response)
    if request.method == 'GET':
        url = str(kube_server+"/api/v1/namespaces/"+ namespace+ "-" + appEnv  +"/configmaps/" + appName)
        response = requests.request(method=request.method,url=url,headers=kube_headers)
        return HttpResponse(response)
    if request.method == "PUT":
        url = str(kube_server+"/api/v1/namespaces/"+ namespace+ "-" + appEnv  +"/configmaps/" + appName)
